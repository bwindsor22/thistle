...
// an ordering of Neighbor of a Point
...
pub struct FlatPoint {
    ...
    p_id: PointId, 
    /// neighbours info
    neighbours:Vec<Neighbor>,
}


impl FlatPoint {
    /// returns the neighbours orderded by distance.
    pub fn get_neighbours(&self) -> &Vec<Neighbor> {
...
fn flatten_point<T:Clone+Send+Sync>(point :&Point<T>) -> FlatPoint {
    ...
    let mut flat_neighbours = Vec::<Neighbor>::new();
...
impl FlatNeighborhood {
    ...
    pub fn get_neighbours(&self, p_id : DataId) -> Option<Vec<Neighbor>> {
