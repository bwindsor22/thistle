        ...
        for i in 0..nb_layer {
            ...
            if nb_ngbh > 0usize {
                ...
                for pointwo in &ref_neighbours[i] {
                    neighbours.push(Neighbor::new(pointwo.point_ref.get_origin_id(),pointwo.dist_to_ref, 
...
impl <T:Clone+Send+Sync, D: Distance<T>+Send+Sync > Hnsw<T,D>  {
   ...
   pub fn insert(&self, data_with_id: (&Vec<T>,usize))  {
    ...
    fn search_general(&self, data :&Vec<T> , knbn:usize, ef_arg:usize) -> Vec<Neighbor> {
        ...
        let knn_neighbours : Vec<Neighbor> = 
                neighbours[0..last].iter().map(|p| Neighbor::new(p.as_ref().point_ref.origin_id, p.as_ref().dist_to_ref, p.as_ref().point_ref.p_id)).collect();
    ...
    fn search_with_id(&self, request : (usize, &Vec<T>) , knbn:usize, ef:usize) -> (usize, Vec<Neighbor>) {
        (request.0, self.search(request.1, knbn, ef))
    }

    /// knbn is the number of nearest neigbours asked for. Returns for each data vector
    /// a Vector of Neighbor
    pub fn parallel_search(&self, datas: &Vec<Vec<T>>, knbn:usize, ef:usize) -> Vec<Vec<Neighbor >> {
